...
func TestNewParser_ContentAuthoritative(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			content:     `[{"a":1}]`,
			tryDecoding: true,
			wantType:    "*blobstream.jsonParser",
		},
		{
			...
			content:     avro,
			tryDecoding: true,
			wantType:    "*blobstream.avroOcfParser",
		},
		{
			...
			tryDecoding: true,
			wantType:    "*blobstream.lineParser",
		},
		{
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			parser, err := newParser(stream, br)
			require.NoError(t, err)
...
func typeName(p LogParser) string {
	switch p.(type) {
	case *jsonParser:
		return "*blobstream.jsonParser"
	case *avroOcfParser:
		return "*blobstream.avroOcfParser"
	case *lineParser:
		return "*blobstream.lineParser"
	default:
		return "unknown"
	}
}
