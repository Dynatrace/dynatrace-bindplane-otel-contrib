...
type Offset struct {
	...
	// Version identifies the object the offset was written for — its ETag (S3) or
	// generation (GCS). Names are reused, so a reader discards an offset whose Version
	// does not match the object it is reading rather than resuming a new object at an
	// old one's position. Empty for legacy offsets; omitempty keeps those byte-identical.
