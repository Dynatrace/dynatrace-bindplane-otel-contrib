...
// newline but a stray carriage return (a CRLF file whose last line lost its '\n') has the
// '\r' trimmed, matching every other line, rather than being delivered as "last\r".
func TestLineParser_FinalUnterminatedLineTrimsCR(t *testing.T) {
	t.Parallel()

	parser := newLineParserFromString("line1\r\nlast\r", 4096)
	...
	require.Equal(t, []string{"line1", "last"}, got,
		"a stray trailing CR on the final unterminated line must be trimmed like every other line")
}
