...
func TestNewArchiveOffset(t *testing.T) {
	...
	o := blobstream.NewArchiveOffset(3, 512)
...
func TestOffset_MarshalUnmarshalArchive(t *testing.T) {
	...
	original := blobstream.NewArchiveOffset(7, 4096)
	data, err := original.Marshal()
	require.NoError(t, err)

	restored := blobstream.NewOffset(0)
...
func TestOffset_NonArchiveMarshalOmitsEntryIndex(t *testing.T) {
	...
	data, err := blobstream.NewOffset(55).Marshal()
	require.NoError(t, err)
	require.JSONEq(t, `{"offset":55}`, string(data))
}

// TestOffset_VersionRoundTrips verifies Version survives marshal/unmarshal and is
// omitted when empty, so legacy blobs stay byte-identical.
func TestOffset_VersionRoundTrips(t *testing.T) {
	t.Parallel()

	o := &blobstream.Offset{Offset: 10, Version: "etag-abc"}
	data, err := o.Marshal()
	require.NoError(t, err)

	restored := blobstream.NewOffset(0)
	require.NoError(t, restored.Unmarshal(data))
	require.Equal(t, "etag-abc", restored.Version)

	data, err = blobstream.NewOffset(10).Marshal()
	require.NoError(t, err)
	require.JSONEq(t, `{"offset":10}`, string(data))
}
...
func TestOffset_MarshalUnmarshal(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			original := blobstream.NewOffset(tc.value)
